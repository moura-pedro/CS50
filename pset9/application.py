...
def buy():
    ...
    if request.method == "POST":
        ...
        if find_missing_errors:
            ...
            apology("invalid number of shares")
...
def register():
    if request.method == "POST":
        ...
        try:
            prim_key = db.execute("INSERT INTO users (username, hash) VALUES (:username, :hash)", username=request.form.get("username"), hash=generate_password_hash(request.form.get("password")))
        except:
